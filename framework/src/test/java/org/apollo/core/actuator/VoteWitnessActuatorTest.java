...
import org.apollo.core.actuator.VoteWitnessActuator;
...
public class VoteWitnessActuatorTest {
  ...
  public void balanceNotSufficient() {
    ...
    try {
      actuator.validate();
      actuator.execute(ret);
      fail("The total number of votes[" + 1000000 + "] is greater than the apolloPower["
      ...
      Assert.assertTrue(e instanceof ContractValidateException);
      Assert
          .assertEquals("The total number of votes[" + 1000000 + "] is greater than the apolloPower["
  ...
  public void voteWitnessWithOldTronPowerAfterNewResourceModel() {
    ...
    try {
      ...
      owner =
          dbManager.getAccountStore().get(ByteArray.fromHexString(OWNER_ADDRESS));
      Assert.assertEquals(2000000L, owner.getInstance().getOldApolloPower());
    } catch (ContractValidateException e) {
      e.printStackTrace();
      Assert.assertFalse(e instanceof ContractValidateException);
    } catch (ContractExeException e) {
  ...
  public void voteWitnessWithOldAndNewTronPowerAfterNewResourceModel() {
    ...
    try {
      ...
      Assert.assertEquals(2000000L, owner.getInstance().getOldApolloPower());
      Assert.assertEquals(1000000L, owner.getInstance().getApolloPower().getFrozenBalance());
    } catch (ContractValidateException e) {
      e.printStackTrace();
      Assert.assertFalse(e instanceof ContractValidateException);
    } catch (ContractExeException e) {
