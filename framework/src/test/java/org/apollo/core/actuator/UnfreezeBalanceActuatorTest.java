...
import org.apollo.core.actuator.UnfreezeBalanceActuator;
...
public class UnfreezeBalanceActuatorTest {
  ...
  private Any getContractForTronPower(String ownerAddress) {
    return Any.pack(UnfreezeBalanceContract.newBuilder()
        .setOwnerAddress(ByteString.copyFrom(ByteArray.fromHexString(ownerAddress)))
        .setResource(ResourceCode.APOLLO_POWER).build());
  }
  ...
  public void testUnfreezeBalanceForEnergyWithOldTronPowerAfterNewResourceModel() {
    ...
    try {
      ...
      Assert.assertEquals(owner.getInstance().getOldApolloPower(), -1L);
    } catch (ContractValidateException e) {
      Assert.assertFalse(e instanceof ContractValidateException);
    } catch (ContractExeException e) {
      Assert.assertFalse(e instanceof ContractExeException);
