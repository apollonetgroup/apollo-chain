  ...
  public boolean validate() throws ContractValidateException {
    ...
    if (!ArrayUtils.isEmpty(receiverAddress) && dynamicStore.supportDR()) {
      ...
      switch (unfreezeBalanceContract.getResource()) {
        ...
        case APOLLO_POWER:
          if (dynamicStore.supportAllowNewResourceModel()) {
            Frozen frozenBalanceForTronPower = accountCapsule.getInstance().getApolloPower();
            if (frozenBalanceForTronPower.getFrozenBalance() <= 0) {
              throw new ContractValidateException("no frozenBalance(ApolloPower)");
            }
            if (frozenBalanceForTronPower.getExpireTime() > now) {
              throw new ContractValidateException("It's not time to unfreeze(ApolloPower).");
            }
          } else {
            throw new ContractValidateException(
                "ResourceCode error.valid ResourceCode[BANDWIDTH、Energy]");
        ...
        default:
          if (dynamicStore.supportAllowNewResourceModel()) {
            throw new ContractValidateException(
                "ResourceCode error.valid ResourceCode[BANDWIDTH、Energy、APOLLO_POWER]");
          } else {
            throw new ContractValidateException(
                "ResourceCode error.valid ResourceCode[BANDWIDTH、Energy]");
          }
