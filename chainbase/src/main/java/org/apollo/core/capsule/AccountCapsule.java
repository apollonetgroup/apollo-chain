...
import org.apollo.protos.Protocol.Account.AccountResource;
import org.apollo.protos.Protocol.Account.Builder;
import org.apollo.protos.Protocol.Account.Frozen;
import org.apollo.protos.Protocol.Permission.PermissionType;
...
public class AccountCapsule implements ProtoCapsule<Account>, Comparable<AccountCapsule> {
  ...
  public long getAllTronPower() {
    if (account.getOldApolloPower() == -1) {
      return getTronPowerFrozenBalance();
    } else if (account.getOldApolloPower() == 0) {
      return getTronPower() + getTronPowerFrozenBalance();
    } else {
      return account.getOldApolloPower() + getTronPowerFrozenBalance();
    }
  }
  ...
  public boolean oldTronPowerIsNotInitialized() {
    return this.account.getOldApolloPower() == 0;
  }

  public boolean oldTronPowerIsInvalid() {
    return this.account.getOldApolloPower() == -1;
  }
  ...
  public void setFrozenForTronPower(long frozenBalance, long expireTime) {
    ...
    setInstance(getInstance().toBuilder()
        .setApolloPower(newFrozen)
        .build());
  }

  public long getTronPowerFrozenBalance() {
    return this.account.getApolloPower().getFrozenBalance();
  }
  ...
  public List<ByteString> getAncestorsList() {
	  return this.account.getAncestorsList();
  }
  
  public void setAncestorsList(List<ByteString> ancestors) {
	  this.account = this.account.toBuilder()
